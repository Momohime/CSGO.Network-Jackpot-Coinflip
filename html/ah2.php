		                    ...
		                    <div class="col-lg-12">
		                    	<div class="widget-bg-color-icon card-box">
									...
									<table id="datatable" class="table table-striped table-bordered">
                                        ...
                                        <tbody>';
										$rs = $conn->query("SELECT * FROM `p2games` ORDER BY id DESC");
										while($row = $rs->fetch_array())
										{
											$winner=$row['winner'];
											$winner=secureoutput($winner);
											$ticket=$row['module']*$row['cost'];
											echo'
											<tr>
												<td>'.$row['id'].'</td>
												<td><a href="profile.php?action=view&id='.$row['userid'].'" target="_BLANK">'.$winner.'</a></td>
												<td>$'.$row['cost'].'</td>
												<td>'.$row['percent'].'%</td>
												<td>'.$row['itemsnum'].'</td>
												<td>'.$row['module'].'</td>
												<td>'.$ticket.'</td>
												<td><a href="pf.php?action=verify&id='.$row['hash'].'" target="_BLANK">'.$row['hash'].'</a></td>
											</tr>
											';
